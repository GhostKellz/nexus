...
pub fn decode(output: []u8, input: []const u8, tree: []const DecodeNode) !usize {
    ...
    for (input, 0..) |byte, byte_idx| {
        ...
        while (bit_idx < 8) : (bit_idx += 1) {
            ...
            if (tree[node_idx].symbol) |symbol| {
                ...
                if (symbol == 255 and byte_idx == input.len - 1) {
                    // This is valid padding - check remaining bits are all 1
                    const remaining_bits = 8 - bit_idx - 1;
                    if (remaining_bits > 0) {
                        const mask: u8 = (@as(u8, 1) << @intCast(remaining_bits)) - 1;
                        const padding = (byte >> @intCast(7 - bit_idx - remaining_bits)) & mask;
                        if (padding != mask) return Error.InvalidPadding;
                    }
                    return out_pos;
                }
    ...
    // Verify we're at root (complete decode)
